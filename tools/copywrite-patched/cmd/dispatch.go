	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(conf.Dispatch.IgnoredRepos) > 0 {
			...
			for _, v := range conf.Dispatch.IgnoredRepos {
				cliLogger.Info(pretty.FgCyan.Sprint(v))
			}
			gha.EndGroup()
		...
		if plan {
			cliLogger.Info(pretty.Bold.Sprint("The following repos would be audited:"))
			for _, v := range targetRepos {
				cliLogger.Info(fmt.Sprintf("%v/%v", conf.Dispatch.GitHubOrgToAudit, *v.Name))
			}
			cliLogger.Info(pretty.FgYellow.Sprintf("Executing in dry-run mode. Rerun without the `--plan` flag to trigger audits on all %v repos.", len(targetRepos)))
			return
		}
		...
		if len(failures) > 0 {
			...
			cobra.CheckErr(fmt.Errorf("%d workflow audits failed", len(failures)))
		}
...
func init() {
	...
	dispatchCmd.Flags().StringP("batch-id", "i", "", "A unique identifier for the current batch of workflow runs (defaults to an autogenerated 8-character hex string)")
