...
func (c *Config) LoadCommandFlags(flagSet *pflag.FlagSet, mapping map[string]string, clobberWithDefaults bool) error {
	...
	flagSet.VisitAll(func(f *pflag.Flag) {
		...
		if !f.Changed && ko != nil && ko.Exists(key) {
			return
		}

		mp[key] = flagValue(flagSet, f)
	})
...
func flagValue(flagSet *pflag.FlagSet, flag *pflag.Flag) interface{} {
	key := flag.Name

	switch flag.Value.Type() {
	case "int":
		value, _ := flagSet.GetInt(key)
		return int64(value)
	case "uint":
		value, _ := flagSet.GetUint(key)
		return uint64(value)
	case "int8":
		value, _ := flagSet.GetInt8(key)
		return int64(value)
	case "uint8":
		value, _ := flagSet.GetUint8(key)
		return uint64(value)
	case "int16":
		value, _ := flagSet.GetInt16(key)
		return int64(value)
	case "uint16":
		value, _ := flagSet.GetUint16(key)
		return uint64(value)
	case "int32":
		value, _ := flagSet.GetInt32(key)
		return int64(value)
	case "uint32":
		value, _ := flagSet.GetUint32(key)
		return uint64(value)
	case "int64":
		value, _ := flagSet.GetInt64(key)
		return value
	case "uint64":
		value, _ := flagSet.GetUint64(key)
		return value
	case "float":
		value, _ := flagSet.GetFloat64(key)
		return value
	case "float32":
		value, _ := flagSet.GetFloat32(key)
		return value
	case "float64":
		value, _ := flagSet.GetFloat64(key)
		return value
	case "bool":
		value, _ := flagSet.GetBool(key)
		return value
	case "duration":
		value, _ := flagSet.GetDuration(key)
		return value
	case "ip":
		value, _ := flagSet.GetIP(key)
		return value
	case "ipMask":
		value, _ := flagSet.GetIPv4Mask(key)
		return value
	case "ipNet":
		value, _ := flagSet.GetIPNet(key)
		return value
	case "count":
		value, _ := flagSet.GetCount(key)
		return value
	case "bytesHex":
		value, _ := flagSet.GetBytesHex(key)
		return value
	case "bytesBase64":
		value, _ := flagSet.GetBytesBase64(key)
		return value
	case "string":
		value, _ := flagSet.GetString(key)
		return value
	case "stringSlice":
		value, _ := flagSet.GetStringSlice(key)
		return value
	case "intSlice":
		value, _ := flagSet.GetIntSlice(key)
		return value
	case "uintSlice":
		value, _ := flagSet.GetUintSlice(key)
		return value
	case "int32Slice":
		value, _ := flagSet.GetInt32Slice(key)
		return value
	case "int64Slice":
		value, _ := flagSet.GetInt64Slice(key)
		return value
	case "float32Slice":
		value, _ := flagSet.GetFloat32Slice(key)
		return value
	case "float64Slice":
		value, _ := flagSet.GetFloat64Slice(key)
		return value
	case "boolSlice":
		value, _ := flagSet.GetBoolSlice(key)
		return value
	case "durationSlice":
		value, _ := flagSet.GetDurationSlice(key)
		return value
	case "ipSlice":
		value, _ := flagSet.GetIPSlice(key)
		return value
	case "stringArray":
		value, _ := flagSet.GetStringArray(key)
		return value
	case "stringToString":
		value, _ := flagSet.GetStringToString(key)
		return value
	case "stringToInt":
		value, _ := flagSet.GetStringToInt(key)
		return value
	case "stringToInt64":
		value, _ := flagSet.GetStringToInt64(key)
		return value
	default:
